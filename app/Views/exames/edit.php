...
<div class="app-container">
    ...
    <main class="main-content">
        <div class="main-container">
            <div class="header">
                <h1><i class="bi bi-pencil"></i> Editar Exame</h1>
                <p class="subtitle">Editar Exame: <strong><?= esc($exame['nome']) ?></strong></p>
            </div>
            <div class="section-card mt-4">
                <form action="<?= base_url('exames/update/' . $exame['id_exame']) ?>" method="POST">
                    <?= csrf_field() ?>
                    <div class="mb-3">
                        <label for="nome" class="form-label">Nome do Exame <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="nome" name="nome" value="<?= old('nome', $exame['nome']) ?>" required>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="codigo" name="codigo" value="<?= old('codigo', $exame['codigo'] ?? '') ?>">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="tipo" name="tipo" required>
                            ...
                            <?php foreach ($tipos_exame as $tipo): ?>
                                <option value="<?= $tipo ?>" <?= old('tipo', $exame['tipo']) == $tipo ? 'selected' : '' ?>><?= ucfirst($tipo) ?></option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="descricao" name="descricao" rows="2"><?= old('descricao', $exame['descricao'] ?? '') ?></textarea>
